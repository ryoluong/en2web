        ...
        <div class="pt-2 pt-lg-3 pb-6">
          <div class="d-flex flex-wrap justify-space-between">
            ...
            >
              ...
              <div class="d-flex mx-2 mx-sm-4" style="overflow-x:scroll;">
                <div v-for="photo in uploadedPhotos" :key="photo.id">
                  <v-checkbox
                    v-model="deletePhotoIds"
                    :value="photo.id"
                    hide-details
                    class="mt-0 mb-2"
                  ></v-checkbox>
          ...
          <v-switch
            v-model="isBest"
            :disabled="submitting"
            class="ml-4 mt-0 mt-sm-2"
            label="ベストノートに指定する"
            type="checkbox"
          />
          <v-btn
            class="white--text ml-4 mt-1 mt-sm-4"
            min-width="100px"
            :loading="submitting"
            color="blue-grey"
            large
...
export default {
  ...
  computed: {
    ...
    params() {
      return {
        ...
        files: this.files,
        delete_photo_ids: this.deletePhotoIds,
      };
    },
  },
  async created() {
    ...
    this.date = note.date;
    this.selectedCountries = note.countries.map(c => c.name);
    this.selectedTags = note.tags.map(t => t.name);
    this.categoryId = note.category_id;
    this.isBest = note.isBest;
    this.content = note.content;
    this.uploadedPhotos = note.photos;
  ...
  methods: {
    ...
    async submit() {
      ...
      const note = await this.updateNote(this.params);
      if (note) {
